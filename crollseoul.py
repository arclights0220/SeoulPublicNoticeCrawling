...
def gangseo(a):
    ...
    soup = BeautifulSoup(html, 'html.parser')
    search_result = soup.select_one(
        '#container > div > div.search-result-wrap > div > div.search-list > section:nth-child(5) > div.search-body > ul > li:nth-child(1) > dl > dt')
    if not search_result:
        print("강서구청 에러")
    else:
        onlytext = search_result.getText()
...
def mapo(a):
    ...
    driver.get(url)
    box = driver.find_element_by_id('sv')
    btn = driver.find_element_by_class_name('btn_midium')
    box.send_keys(a)
    btn.click()
    html = driver.page_source
    soup = BeautifulSoup(html, 'html.parser')
    search_result = soup.select_one(
        '#container > div.contents > div.bbs_list.bbs_list_type4by14 > table > tbody > tr:nth-child(6)')
    if not search_result:
        print("\n마포구청 " + a + "에 관한 최신공고 : ")
    else:
        onlytext = search_result.getText()
...
def sdm(a):
    ...
    html = driver.page_source
    soup = BeautifulSoup(html, 'html.parser')
    search_result = soup.select_one('#frm > table > tbody > tr:nth-child(1)')
    if not search_result:
        print("서대문구청 에러")
    else:
        onlytext = search_result.getText()
...
def ydp(a):
    ...
    btn = driver.find_element_by_xpath("/html/body/div[2]/div[2]/div/main/article/div/div[2]/form/fieldset/div/div/span[2]/button")
    box.send_keys(a)
    btn.click()
    html = driver.page_source
    soup = BeautifulSoup(html, 'html.parser')
...
def ep(a):
    ...
    btn = driver.find_element_by_xpath('/html/body/div/div/div/main/article/div/div[1]/form/div/div[2]/button')
    box.send_keys(a)
    btn.click()
    html = driver.page_source
    soup = BeautifulSoup(html, 'html.parser')
...
what = input("검색할 문장 혹은 단어를 입력하세요 : ")
